...
`arguments` 表示当 `commands`, `flags` 被匹配和处理完毕之后，命令行的剩余参数列表。例如你需要一个像 gcc 一样所的文件列表。在 `cmdr` 中，往往会将其称作 `remained args`，你可以在 Action 所定义的回调函数中直接取得这个数组：
